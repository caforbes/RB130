...
class PhraseTest < Minitest::Test
  ...
  def test_final_apostrophe_extra
  ...
  def test_final_apostrophe_extra # but maybe we don't like this
    phrase = Phrase.new("use ruby_method")
    counts = {
      "use" => 1, 'ruby' => 1, 'method' => 1
    }
    assert_equal counts, phrase.word_count
  end

  def test_final_apostrophe_extra
    phrase = Phrase.new("we'd've gone")
    counts = {
      "we'd've" => 1, 'gone' => 1
    }
    assert_equal counts, phrase.word_count
  ...
  def test_quotes_at_front
    phrase = Phrase.new("'help' is an important word")
    counts = {
      'help' => 1, 'is' => 1, 'an' => 1, 'important' => 1, 'word' => 1
    }
    assert_equal counts, phrase.word_count
  end
end